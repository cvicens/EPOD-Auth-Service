...
module.exports = function(mediator) {
  ...
  var topicVehicleLoad = 'wfm:user:vehicle:read';
  console.log('Subscribing to mediator topic:', topicVehicleLoad);
  mediator.subscribe(topicVehicleLoad, function(vehicle) {
    setTimeout(function() {
      var user = _.find(users, function(_user) {
        return _user.vehicle === vehicle;
      });
      if (user) {
        mediator.publish('done:' + topicVehicleLoad + ':' + vehicle, user);
      } else {
        mediator.publish('error:' + topicVehicleLoad + ':' + vehicle, 'No such vehicle');
      }
    }, 0);
  });
